...
export class ItemService {
  ...
  getbnbprice(): Observable<any> {
    const url = config.converstion_url;
    return this.http.get<any>(url);
  }

  getbemdprice(): Observable<any> {
    const url = config.bemd_conv;
    return this.http.get<any>(url);
  }
  ...
  updateItemPrice(params:any): Observable<Common> {
    let url = API.base_url + API.item_updatePrice;
    return this.http.post<Common>(url, params).pipe(map((param:any) => new Common().deserialize(param)));
  }
  ...
  transferItem(params:any): Observable<Common> {
    let url = API.base_url + API.item_transfer;
    return this.http.post<Common>(url, params).pipe(map((param:any) => new Common().deserialize(param)));
  }
