...
exports.add = function(req,res) {
    ...
    item.type = req.body.type ? req.body.type : '';
...
/*
* This is the function which used to transfer item in ethereum network
*/
exports.transfer = function(req,res) {
    ...
    items.findOne({_id:req.body.item_id, status:"active"}).populate('collection_id').exec(function (err, item) {
        ...
        userController.getUserInfoByID(item.current_owner,function(err,receiver){
                var sender_address = req.body.transferaddress;
                var receiver_address = receiver.metamask_info;
                        var symbolabi = item.collection_id.contract_symbol.replace(" ", "_")+'.abi';
                        var command = 'sh transaction.sh '+ receiver_address.id +' '+ sender_address +' '+item.token_id + ' ' + item.collection_id.contract_address + ' ' + symbolabi +' '+  config.owner.key +' ' + config.rpcurl;
		                ...
		                item.transfer = "active";
                                item.save(function (err ,itemObj) {
                                    if (err) {
                                        ...
                                        res.json({
                                            status: true,
                                            message: "Item transferred out successfully",
                                            result: itemObj 
                                        });  
                                    }
                                });
            ...
            userController.getUserInfoByID(req.decoded.user_id,function(err,sender){
                ...
                this.transferAdminComission(item, function(error, comission){
                    this.transferBalance(sender,receiver, item, comission, function(is_transferred){
                        ...
                        var command = 'sh transaction.sh '+ receiver_address.id +' '+ sender_address.id +' '+item.token_id + ' ' + item.collection_id.contract_address + ' ' + symbolabi +' '+  config.owner.key +' ' + config.rpcurl;
                        cp.exec(command, function(err, stdout, stderr) {
                            console.log('stderr ',stderr)
                            console.log('stdout ',stdout)
                            // handle err, stdout, stderr
...
exports.list = function(req,res) {
    ...
    if(req.query.type != "view") { 
        ...
        query = query.populate({path: 'collection_id', model: collections }).populate({path: 'category_id', model: category }).populate({path: 'current_owner', model: users, select:'_id username first_name last_name profile_image metamask_info'})
        options = {
            page:page,
            offset:offset,
            limit:10,    
...
exports.generateHash = function(req,res) {
    ...
    cp.exec(command, function(err, stdout, stderr) {
        ...
        fs.readFile('/root/source/cryptotrade1.3/cryptotrade1.3/backend/'+symbol+'.bin', 'utf8' , (err, data) => {
            if (err) {
              res.json({
                  status: false,
                  message: err.toString().split('ERROR: ').pop().replace(/\n|\r/g, "")
...
exports.view = function(req,res) {
    collections.findOne({name:req.params.id.replace("_"," ")}).exec( function (err, collection) {
        if (err) {
            res.json({
                status: false,
                message: "Request failed",
        ...
        if(!req.params.tokenid) {
            res.json({
                ...
                "external_url": "https://app.bettermentdigital.co/collection/view/"+collection._id, 
                "image": "https://api.bettermentdigital.co/images/collection/logo/"+collection.image, 
                "name": collection.name
            });
        }else{
         var token = req.params.tokenid;
         let tid = Number(token).toString(16).padStart(64, '0');
items.findOne({collection_id:collection._id, token_id:"0x"+tid}).exec( function (err, item) {
                if (err) {
                    res.json({
                        status: false,
                        message: "Request failed",
            ...
            res.json({
                "name": item.name,
                "image": "https://api.bettermentdigital.co/images/item/media/"+item.media,
